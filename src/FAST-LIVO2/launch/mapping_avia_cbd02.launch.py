...
def generate_launch_description():
    ...
    config = os.path.join(pkg_share, "config", "avia_cbd02.yaml")
    camera = os.path.join(pkg_share, "config", "camera_pinhole_cbd02.yaml")
    rviz_config = os.path.join(pkg_share, "rviz_cfg", "fast_livo2.rviz")

    return LaunchDescription([
        DeclareLaunchArgument("rviz", default_value="true"),
        DeclareLaunchArgument("republish", default_value="false"),
        Node(
            package="fast_livo",
