...
void Preprocess::odin_handler(const sensor_msgs::PointCloud2::ConstSharedPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  {
    if (field.name == "offset_time" && field.datatype == sensor_msgs::msg::PointField::FLOAT32)
    {
      has_offset_time = true;
    }
    if (field.name == "confidence" && field.datatype == sensor_msgs::msg::PointField::UINT16)
    {
      has_confidence = true;
    }
  }
  if (!has_offset_time)
  {
    ROS_WARN("Odin cloud_raw has no float32 offset_time field; per-point time is set to 0");
  }
  ...
  std::unique_ptr<sensor_msgs::PointCloud2ConstIterator<uint16_t>> iter_confidence;
  std::unique_ptr<sensor_msgs::PointCloud2ConstIterator<float>> iter_offset_time;
  if (has_confidence) iter_confidence = std::make_unique<sensor_msgs::PointCloud2ConstIterator<uint16_t>>(*msg, "confidence");
  if (has_offset_time) iter_offset_time = std::make_unique<sensor_msgs::PointCloud2ConstIterator<float>>(*msg, "offset_time");

  for (size_t i = 0; i < point_count; ++i, ++iter_x, ++iter_y, ++iter_z, ++iter_intensity)
  {
    const bool keep_stride = (i % static_cast<size_t>(std::max(1, point_filter_num))) == 0;
    const float x = *iter_x;
    const float y = *iter_y;
    const float z = *iter_z;
    const float range2 = x * x + y * y + z * z;
    const uint16_t confidence = iter_confidence ? **iter_confidence : 1;
    const float offset_time_s = iter_offset_time ? **iter_offset_time : 0.0f;

    if (iter_confidence) ++(*iter_confidence);
    if (iter_offset_time) ++(*iter_offset_time);

    if (!keep_stride || confidence == 0 || range2 < blind * blind) continue;
    ...
    added_pt.y = y;
    added_pt.z = z;
    added_pt.intensity = static_cast<float>(*iter_intensity);
    added_pt.normal_x = 0;
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    added_pt.curvature = std::isfinite(offset_time_s) && offset_time_s >= 0.0f ? offset_time_s * 1000.0f : 0.0f;
...
void Preprocess::l515_handler(const sensor_msgs::PointCloud2::ConstSharedPtr &msg)
{
  ...
  double time_stamp = fast_livo::stampToSec(msg->header.stamp);
  // cout << "===================================" << endl;
