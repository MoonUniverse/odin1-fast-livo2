...
{
  readParam(node, "publish.pub_plane_en", voxel_config.is_pub_plane_map_, false);
  
  readParam(node, "lio.max_layer", voxel_config.max_layer_, 1);
  readParam(node, "lio.voxel_size", voxel_config.max_voxel_size_, 0.5);
  readParam(node, "lio.min_eigen_value", voxel_config.planner_threshold_, 0.01);
  readParam(node, "lio.sigma_num", voxel_config.sigma_num_, 3.0);
  readParam(node, "lio.beam_err", voxel_config.beam_err_, 0.02);
  readParam(node, "lio.dept_err", voxel_config.dept_err_, 0.05);
  std::vector<int64_t> layer_init_num;
  readParam(node, "lio.layer_init_num", layer_init_num, std::vector<int64_t>{5, 5, 5, 5, 5});
  voxel_config.layer_init_num_.assign(layer_init_num.begin(), layer_init_num.end());
  readParam(node, "lio.max_points_num", voxel_config.max_points_num_, 50);
  readParam(node, "lio.max_iterations", voxel_config.max_iterations_, 5);

  readParam(node, "local_map.map_sliding_en", voxel_config.map_sliding_en, false);
  readParam(node, "local_map.half_map_size", voxel_config.half_map_size, 100);
  readParam(node, "local_map.sliding_thresh", voxel_config.sliding_thresh, 8.0);
}
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount == config_setting_.max_iterations_ - 1)))
    {
      ...
      Eigen::Quaterniond quat_cur(state_.rot_end);
      geoQuat_.x = quat_cur.x();
      geoQuat_.y = quat_cur.y();
      geoQuat_.z = quat_cur.z();
      geoQuat_.w = quat_cur.w();
