...
void VIOManager::retrieveFromVisualSparseMap(cv::Mat img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  // "<<t_5<<endl;
  if (g_fast_livo_verbose) printf("[ VIO ] Retrieve %d points from visual sparse map\n", total_points);
}
...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar> &pg)
{
  ...
  if (g_fast_livo_verbose) printf("[ VIO ] Append %d new visual map points\n", add);
  // printf("pg.size: %d \n", pg.size());
  // printf("B1. : %.6lf \n", t_b1);
  // printf("B2. : %.6lf \n", t_b2);
}
...
void VIOManager::updateVisualMapPoints(cv::Mat img)
{
  ...
  if (g_fast_livo_verbose) printf("[ VIO ] Update %d points in visual submap\n", update_num);
}
...
void VIOManager::projectPatchFromRefToCur(const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  int patch_size = 25;
  string dir = DEBUG_FILE_DIR("ref_cur_combine/");
...
void VIOManager::dumpDataForColmap()
{
  ...
  std::string image_path = DEBUG_FILE_DIR(std::string("Colmap/images/") + cnt_str + ".png");
